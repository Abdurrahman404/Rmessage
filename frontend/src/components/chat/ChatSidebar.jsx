...
function ChatSidebar() {
  ...
  const filterUsers = (items) => {
    const mappedItems = items.map((user) => mapUserForList(user, onlineUsers));
    return normalizedSearchQuery
      ? mappedItems.filter((user) => user.name.toLowerCase().includes(normalizedSearchQuery))
      : mappedItems;
  };

  const filteredConversations = filterUsers(conversations);
    ...
    >
      <div className="shrink-0 border-b border-border px-2 pb-2 pt-2.5 sm:px-3 sm:pt-3">
        <div className="flex items-center gap-2 px-0.5 sm:gap-2.5 sm:px-1">
          ...
          <p className="flex-1 truncate text-lg font-bold tracking-tight sm:text-[22px]">{APP_NAME}</p>
      ...
      >
        ...
        <Tabs.ListContainer className="shrink-0 border-b border-border px-2 pb-2 pt-1">
          <Tabs.List className="w-full gap-0.5">
            <Tabs.Tab id="chats" className="flex-1 justify-center gap-1.5"><MessageSquareIcon className="size-3.5 opacity-80" />Chats</Tabs.Tab>
            <Tabs.Tab id="users" className="flex-1 justify-center gap-1.5"><UsersIcon className="size-3.5 opacity-80" />Users</Tabs.Tab>
          </Tabs.List>
        </Tabs.ListContainer>
        <Tabs.Panel id="chats" className="flex-1 overflow-x-hidden overflow-y-auto outline-none">
          {filteredConversations.length === 0 ? <p className="px-4 py-6 text-center text-sm text-muted">No conversations match your search.</p> : filteredConversations.map((user) => <ConversationRow key={user.id} user={user} selected={user.id === activeConversationId} onSelect={() => setActiveConversationId(user.id)} />)}
        </Tabs.Panel>
        <Tabs.Panel id="users" className="flex-1 overflow-x-hidden overflow-y-auto outline-none">
          {filteredUsers.length === 0 ? <p className="px-4 py-6 text-center text-sm text-muted">No people match your search.</p> : filteredUsers.map((user) => <ConversationRow key={user.id} user={user} selected={user.id === activeConversationId} onSelect={() => setActiveConversationId(user.id)} />)}
        </Tabs.Panel>
      </Tabs>
    </aside>
  );
